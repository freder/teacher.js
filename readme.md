...
- synchronization between the client with the _instructor role_ and all other clients via websockets, so that students will always see what the teacher sees:
	- [reveal.js](https://revealjs.com/) presentations
		- connected clients notified about slide changes, etc.
	- wikipedia pages
		- connected clients notified about changes (page transitions, jumping to specific sections within the page, etc.)
- recording of teaching sessions (to be replayed at a later time)
	...
	- relevant events (caused by the presentation or wikipedia module) are time-stamped and logged to a file on the server
